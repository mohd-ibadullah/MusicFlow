...
const parseExpiryMs = () => {
  const raw = import.meta.env.VITE_LOOP_INTERVENTION_EXPIRY_MS;
  const parsed = Number.parseInt(String(raw || "").trim(), 10);
  if (Number.isFinite(parsed) && parsed >= 5000 && parsed <= 600000) {
    return parsed;
  }
  // Align with server default LOOP_INTERVENTION_EXPIRY_SECONDS (120) unless overridden.
  return 120000;
};

const EXPIRY_MS = parseExpiryMs();
...
export function LoopInterventionCard({
  ...
  }, [normalizedIntervention, handleIgnore]);
  ...
  const snoozeLabel = useMemo(() => {
    ...
    if (!Number.isFinite(seconds) || seconds <= 0) {
      return "Snooze reminders";
    }
    if (seconds < 120) {
      return `Snooze ${Math.round(seconds)} sec`;
    }
    const minutes = Math.round(seconds / 60);
    return minutes >= 1 ? `Snooze ${minutes} min` : "Snooze reminders";
  }, [normalizedIntervention]);
