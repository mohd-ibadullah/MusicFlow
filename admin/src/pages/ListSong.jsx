...
const ListSong = () => {
  ...
  const removeSong = async (id, songName) => {
    if (!window.confirm(`Are you sure you want to delete "${songName}"?`)) return;
    try {
      ...
      if (response.data.success) {
        toast.success(`"${songName}" deleted`);
        await fetchSongs();
      } else {
    ...
    <div className="w-full animate-fade-in">
      ...
      {data.length === 0 ? (
        <div className="card-admin text-center py-16 px-6">
          ...
          <p className="text-gray-800 font-medium mb-1">No songs yet</p>
          <p className="text-sm text-gray-500">Add your first song to get started</p>
        </div>
      ) : (
