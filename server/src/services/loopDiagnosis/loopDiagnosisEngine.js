...
// SYSTEM SCAN: io is attached via app.set("io", io) in server/server.js and should be used additively.
...
export function createLoopDiagnosisEngine(overrides = {}) {
  ...
  return async function handlePlayEvent({ userId, songId, redisClient, io }) {
    ...
    try {
      ...
      // Per-song replay window (for loop detection) vs continuous listening clock (for wellbeing session nudges).
      const blendedSessionMinutes = Math.max(perSongWindowMinutes, continuousSessionMinutes);

      logger.debug(
        `[LD][SESSION] user=${userId} sessionStart=${continuousSessionStart.toISOString()} now=${now.toISOString()} songWindowMinutes=${perSongWindowMinutes.toFixed(2)} continuousSessionMinutes=${continuousSessionMinutes.toFixed(2)} blendedSessionMinutes=${blendedSessionMinutes.toFixed(2)}`
      );
      ...
      const behaviorProfile = await deps.buildBehaviorProfile({
        userId,
        loopedSong,
        sessionDurationMinutes: blendedSessionMinutes,
      });
      ...
      // Session / health nudges must follow the continuous listening clock, not the per-song replay window.
      const sessionTriggered =
        continuousSessionMinutes >= adjustedSessionThreshold &&
        (!shouldRespectQuietHours || !inQuietHours);

      logger.debug(
        `[LD][TRIGGER_CHECK] user=${userId} loopTriggered=${loopTriggered} sessionTriggered=${sessionTriggered} continuousSessionMinutes=${continuousSessionMinutes.toFixed(2)} threshold=${adjustedSessionThreshold} baseThreshold=${baseSessionThreshold} inQuietHours=${inQuietHours} lateNight=${isLateNightContext} reminderStyle=${reminderStyle}`
      );
      ...
      if (!loopTriggered && !sessionTriggered) {
        ...
        if (!sessionTriggered) {
          ...
          if (continuousSessionMinutes < adjustedSessionThreshold) {
            skipReasons.push(
              `session=false (${continuousSessionMinutes.toFixed(2)}m < ${adjustedSessionThreshold}m threshold)`
            );
          }
        }

        logger.debug(
          `[LD] No wellbeing trigger. loop=${loopTriggered} session=${sessionTriggered} continuousSessionMinutes=${continuousSessionMinutes} reasons=${skipReasons.join("; ")}`
        );
        return;
      }
      ...
      if (candidates.length === 0 && triggerCategory === "loop") {
        ...
        try {
          await deps.LoopEvent.create({
            ...
            sessionDurationMinutes: continuousSessionMinutes,
      ...
      const behaviorContext = {
        ...
        timeOfDay: deps.getTimeOfDay(hour),
        isLateNight: deps.isLateNight(hour, config),
        sessionWindowMinutes: blendedSessionMinutes,
        fatigueLevel: behaviorProfile.fatigueLevel,
